/**
 * Server-side authentication for MRP protocol.
 * Simulates an Apple TV's pairing and verification endpoints.
...
export abstract class MrpServerAuth {
	private _keys: ServerKeys;
	private _session: SRPServerSession | null = null;
	private _pin: number;
	...
	get serverIdentifier(): Buffer {
		return this._keys.publicKey;
	}

	abstract sendToClient(data: Buffer): Promise<void>;
	abstract enableEncryption(outputKey: Buffer, inputKey: Buffer): void;

	async handlePairSetup(pairingData: Buffer): Promise<Buffer> {
	...
	async handlePairVerify(pairingData: Buffer): Promise<Buffer> {
		const tlv = readTlv(pairingData);
		const seqNo = tlv.get(TlvValue.SeqNo)?.[0];
	...
	private _pairSetupM3(tlv: Map<number, Buffer>): Buffer {
		...
		if (!verified) {
			...
			resp.set(TlvValue.Error, Buffer.from([0x02])); // Authentication error
			return writeTlv(resp);
		}
	...
	private _pairSetupM5(tlv: Map<number, Buffer>): Buffer {
		...
		const _clientPubKey = innerTlv.get(TlvValue.PublicKey) ?? null;
		...
		const serverId = Buffer.from("MrpServerAuth");
	...
	private _verifyShared: Buffer | null = null;
	...
	private _pairVerifyM3(tlv: Map<number, Buffer>): Buffer {
		...
		const _innerTlv = readTlv(decrypted);
