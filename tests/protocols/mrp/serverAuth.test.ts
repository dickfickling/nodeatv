...
class TestMrpServerAuth extends MrpServerAuth {
	...
	async sendToClient(data: Buffer): Promise<void> {
...
describe("MrpServerAuth", () => {
	it("handles full pair-setup flow", async () => {
		...
		const encryptedDeviceInfo = client.step3();
		const m5Tlv = new Map<number, Buffer>();
		m5Tlv.set(TlvValue.SeqNo, Buffer.from([0x05]));
		m5Tlv.set(TlvValue.EncryptedData, encryptedDeviceInfo);
		...
		// Client decrypts to get credentials
	...
	it("handles pair-verify flow", async () => {
		// First do pair-setup to get credentials
		const server = new TestMrpServerAuth(1111);
		const client = new SRPAuthHandler();
		client.initialize();
		...
		expect(v2Tlv.has(TlvValue.PublicKey)).toBe(true);
		expect(v2Tlv.has(TlvValue.EncryptedData)).toBe(true);
	...
	it("rejects wrong PIN", async () => {
		...
		expect(m4Tlv.has(TlvValue.Error)).toBe(true);
	});
});
